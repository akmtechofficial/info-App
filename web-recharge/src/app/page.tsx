...
function RechargeWebPageContent() {
  ...
  // PAYFLUX PAYMENT GATEWAY RECHARGE WORKFLOW
  const handleInitiateRechargeAndRedirect = async (amount: number, credits: number) => {
    ...
    try {
      ...
      if (data.success && data.checkoutUrl) {
        // Redirect user to Payflux Payment Checkout Page
        window.location.href = data.checkoutUrl;
      } else {
        throw new Error(data.error || "Failed to initialize payment gateway order with Payflux.");
      }
    } catch (err: unknown) {
      const error = err as Error;
      console.error("Payflux recharge error:", error);
      setErrorNotice("Payment Error: " + (error.message || "Unable to open payment gateway"));
